...
class SwipeableCardsSection extends StatefulWidget {
  ...
  SwipeableCardsSection({
    ...
    this.cardHeightTopMul = 0.65,
    this.cardHeightMiddleMul = 0.65,
    this.cardHeightBottomMul = 0.65,
...
class _CardsSectionState extends State<SwipeableCardsSection>
  ...
  final Alignment defaultFrontCardAlign = const Alignment(0.0, -0.10);
  Alignment frontCardAlign = cardsAlign[2];
  double frontCardRot = 0.0;

  void _triggerSwipe(Direction dir) {
    final swipedCallback = widget.onCardSwiped ?? (dynamic _, dynamic __, dynamic ___) => true;
    bool? shouldAnimate = false;
    if (dir == Direction.left) {
      shouldAnimate = swipedCallback(Direction.left, index, widget.items[0]);
      frontCardAlign = const Alignment(-0.001, 0.0);
    } else if (dir == Direction.right) {
      shouldAnimate = swipedCallback(Direction.right, index, widget.items[0]);
      frontCardAlign = const Alignment(0.001, 0.0);
    } /*else if (dir == Direction.up) {
      shouldAnimate = swipedCallback(Direction.up, index, cards[0]);
      frontCardAlign = Alignment(0.0, -0.001);}*/
    else if (dir == Direction.down) {
      shouldAnimate = swipedCallback(Direction.down, index, widget.items[0]);
  ...
  void initState() {
    ...
    final SwipeableCardSectionController? cardController = widget.cardController;
    if (cardController != null) {
      cardController.listener = _triggerSwipe;
      cardController.addItem = _appendItem;
      cardController.update = _updateItem;
      cardController.enableSwipeListener = _enableSwipe;
    }

    // Init cards
    // cards.addAll(widget.items);
          ...
          ((_controller.status != AnimationStatus.forward))
              ? SizedBox.expand(
                  ...
                  onVerticalDragUpdate: (DragUpdateDetails details) {
                    if (details.delta.dy > 0) {
                      setState(() {
                        ...
                        frontCardRot =
                            frontCardAlign.x; // Adjust rotation if needed
                      });
                      if(frontCardAlign.y > 1){
                        Get.find<QuestionScreenController>().isDown.value = true;
                      }else{
                        Get.find<QuestionScreenController>().isDown.value = false;
                      }
                    }
                    // Check if the card has moved down enough to trigger dismissal
                  },
                  onVerticalDragEnd: (_) {
                    // If the front card was swiped far enough to count as swiped

                    final onCardSwiped =
                        widget.onCardSwiped ?? (dynamic _, dynamic __, dynamic ___) => true;
                    bool? shouldAnimate = false;

                    if (frontCardAlign.y > 4.5 && widget.enableSwipeDown) {
                      shouldAnimate =
                          onCardSwiped(Direction.down, index, widget.items[0]);

                      if (shouldAnimate ?? true) {
                        animateCards();
                      }
                    } else {
                      // Reset position if not swiped down enough
                      setState(() {
                  ...
                  onHorizontalDragUpdate: (DragUpdateDetails details) {
                    ...
                    if (kDebugMode) {
                      print("${frontCardAlign.x.isNegative}");
                    }

                    if (frontCardAlign.x > 2.0) {
                      Get.find<QuestionScreenController>().isRight.value = true;
                      Get.find<QuestionScreenController>().isLeft.value = false;
                    } else if (frontCardAlign.x < -2.0) {
                      Get.find<QuestionScreenController>().isLeft.value = true;
                      Get.find<QuestionScreenController>().isRight.value = false;
                    } else {
                      Get.find<QuestionScreenController>().isLeft.value = false;
                      Get.find<QuestionScreenController>().isRight.value = false;
                    }
                  ...
                  onHorizontalDragEnd: (_) {
                    ...
                    final onCardSwiped =
                        widget.onCardSwiped ?? (dynamic _, dynamic __, dynamic ___) => true;
                    bool? shouldAnimate = false;
                    if (frontCardAlign.x > 10.0) {
                      shouldAnimate =
                          onCardSwiped(Direction.right, index, widget.items[0]);
                    } else if (frontCardAlign.x < -10.0) {
                      shouldAnimate =
                          onCardSwiped(Direction.left, index, widget.items[0]);
  ...
  Widget frontCard() {
    return Align(
        ...
        child: Transform.rotate(
          angle: (pi / 360.0) * frontCardRot,
          child: SizedBox.fromSize(size: cardsSize[0], child: widget.items[0]),
        ));
  }
  ...
  void changeCardsOrder() {
    setState(() {
      ...
      // Get.find<HomeScreenCtrl>().feedList.removeAt(0);
      cardsCounter++;
      index++;
      frontCardAlign = defaultFrontCardAlign;
      frontCardRot = 0.0;
